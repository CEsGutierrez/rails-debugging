class RsvpsController < ApplicationController
  ...
  def destroy
    ...
    Rsvp.transaction do
      # @rsvp.destroy
      @rsvp.delete
      WaitlistManager.new(@event.reload).reorder_waitlist! # The issue here is there is that @event is defined as being @rsvp.event, which means that when @rsvp is destroyed, that touches all the other things attached to it. I think I want to change it to .delete so that @event sticks around but that seems dangerous because we always destroy rather than deleting...and yet it works for now. 
    end
